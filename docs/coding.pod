...
containing data managed by L<Smolder::DB::Project> is called C<project> not
C<projects>.
...
C<developer_project> because L<Smolder::DB::Project> is responsible for
...
All C<VARCHAR> columns must be of the maximum allowed width (255 in MySQL)
unless there is a compelling reason to do otherwise. C<VARCHAR>s
...
should be of the C<TEXT> type.  This will avoid unnecessary restrictions
