...
=item 2 - Detailed view of each test result, showing all test files run and which tests passed or failed.
...
We will provide modules to at least convert TAP to XML, and upload it to a running
...
them (since this might require other steps other than simply calling prove, etc).
...
=item * Architecture and Platform

=item * Project Defined categories
