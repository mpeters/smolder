...
Make it easier for custom installation on existing Apache
...
=item Allow for different locations for smolder.conf

=item Add a PrefixURL directive in case we're behind a simple proxy or configured for a different url

=back

=item * 

Use Archive::Extract to support more than just tar

=item *

Add support for plain TAP (using TAPx::Parser)

=item *

RSS in addition to email (XML::RSS?)

=item *

Add ability to not use SMTPServer but straight sendmail
...
Upgrade Prototype.js and Scriptaculous to latest versions
