...
+ add --check_all to smolder_src_dependency_check

+ fix some POD style issues leftover from Krang

+ top level Makefile for common tasks (make test, make db, etc)

+ searchable HTML docs

+ make sure backup works
    + install from backup too
...
+ make sure it buitls/runs/installs on as many *nix platforms as possible
