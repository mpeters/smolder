...
=item * Review upgrade procedure to make sure it's working completely

=item * Add support for plain TAP

=item * Add support for compressed uploads

=item * allow users to have a messages-per-day limit to control out of control failures
...
=item * Precompile all templates at startup (speeds it up and increases shared memory)

=item * Rendering in other browsers besides FF and KHTML

=item * searchable HTML docs

=item * import/export projects (similar to backup and install from backup but by project)
...
=item * optional logging of critical problems to email?
