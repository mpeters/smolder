...
=item * Handle skip-all better in the plan.
...
=item * Allow anonymous uploads via smolder_smoke_signal
...
We'd need GUIDs for tests so maybe we use a custom TAP diagnostic key
...
Allow uploading and authorization all in 1 step and 1 url.
...
Replace image based graphs with browser based (canvas? Google graphs? Plotr?)
