...
The full documentation for TAP can be found at http://search.cpan.org/perldoc?TAP with more information available at http://testanything.org.

The basic idea is that a single file of TAP output is equivalent to a
single test script or module. These files are collected together in a tar
archive file (optionally compressed with gzip). This archive file can
have an arbitrary number of these TAP files (with a F<.tap> extension)
...
This is simply just a list of the files from your test suite in the order you desire. This will usually be the order in which they are run.

=item * start_time

The time the test suite started to run. This time is given in epoch seconds (seconds since 01/01/1970).

=item * stop_time

The time the test suite finished it's run. This time is given in epoch seconds (seconds since 01/01/1970).
...
a F<meta.yml> file to go along with it.
    ...
    # get every .t file in the t/ directory and run in it
    my @files = glob('t/*.t');
    my @tap_files;
    foreach my $file (@files) {
        my $tap_file = basename($file, '.t') . '.tap';
        system("perl $file > $tap_file");
        push(@tap_files, $tap_file);
    }

    my $stop = time();

    my %meta = (
        file_order => \@tap_files,
        start_time => $start,
        stop_time  => $stop,
    );

    open(my $meta_fh, '>', 'meta.yml')
        or die "Could not open meta.yml for writing: $!";
    print $meta_fh YAML::Dump(\%meta);
    close($meta_fh);

    system('tar -zc *.tap meta.yml > test_run.tar.gz') == 0
        or die "Could not use tar to create test run file: $!";
